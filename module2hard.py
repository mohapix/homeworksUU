...
def pairs_generator(number_current):
    divisor_list = []
    for i in range(2, number_current + 1):
        if number_current % i == 0:
            divisor_list.append(i)
    # print("Список делителей:", divisor_list)

    # number_pairs_dict = {}
    result_str = ''
    for divisor_current in divisor_list:
        divisor_current_list = []
        for i in range(1, divisor_current + 1):
            if i > divisor_current // 2:
                break
            divisor_current_list.append(str(i) + str(divisor_current - i))
            result_str += str(i) + str(divisor_current - i)
        # number_pairs_dict.update({divisor_current: divisor_current_list})
    # return result_str, number_pairs_dict
    return result_str


# for number_main in range(3, 21):
#     result, result_dict = pairs_generator(number_main)
#     print("Варианты пар для пароля.\nЛегенда - делитель: [список пар]")
#     print(result_dict)
#     print(f"Число: {number_main}, пароль: {result}")
#     print()
print("Результат:", pairs_generator(number_main))
